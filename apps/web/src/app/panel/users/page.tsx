...
import {
  ...
  IconButton, Chip, Box, Select, MenuItem, FormControl, InputLabel, Alert, Paper,
...
export default function UsersPage() {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'balance', headerName: 'موجودی', width: 130, type: 'number', renderCell: p => <span style={{ color: p.value < 0 ? '#ef4444' : '#16a34a' }}>{p.value?.toLocaleString('fa-IR')} تومان</span> },
    ...
    <Container maxWidth="lg" sx={{ py: 3 }}>
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        <Stack direction="row" sx={{flexWrap:"wrap",alignItems:"center"}}>
          <TextField size="small" placeholder="جستجو..." value={search} onChange={e => setSearch(e.target.value)} sx={{ minWidth: 250 }} slotProps={{ input: { startAdornment: <Search sx={{ mr: 1, color: 'grey.400' }} fontSize="small" /> } }} />
          ...
          <Button variant="outlined" size="small" startIcon={<FilterList />}>فیلتر پیشرفته</Button>
        </Stack>
      </Paper>
        ...
        <Dialog open onClose={() => setShowDetail(null)} maxWidth="md" fullWidth>
          <DialogTitle>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <Avatar sx={{ bgcolor: 'primary.main' }}>{showDetail.name[0]}</Avatar>
              <Box>
                <Typography sx={{ fontWeight: 700 }}>{showDetail.name} {showDetail.family}</Typography>
                <Typography variant="body2" color="text.secondary">{showDetail.uid}</Typography>
              </Box>
            </Box>
          </DialogTitle>
          <DialogContent dividers>
            <Stack direction="row" sx={{flexWrap:"wrap"}} >
              <Box sx={{ flex: '1 1 200px' }}>
                <Typography variant="body2" color="text.secondary">گروه کاربری</Typography>
                <Typography sx={{ fontWeight: 600 }}>{USER_GROUPS.find(g => g.id === showDetail.group)?.name}</Typography>
              </Box>
              <Box sx={{ flex: '1 1 200px' }}>
                <Typography variant="body2" color="text.secondary">سطح دسترسی</Typography>
                <Typography sx={{ fontWeight: 600 }}>{ACCESS_ROLES.find(r => r.id === showDetail.role)?.name}</Typography>
              </Box>
              <Box sx={{ flex: '1 1 200px' }}>
                <Typography variant="body2" color="text.secondary">دانشکده</Typography>
                <Typography sx={{ fontWeight: 600 }}>{FACULTIES.find(f => f.id === showDetail.faculty)?.name}</Typography>
              </Box>
              <Box sx={{ flex: '1 1 200px' }}>
                <Typography variant="body2" color="text.secondary">موجودی</Typography>
                <Typography sx={{ fontWeight: 600, color: showDetail.balance >= 0 ? 'success.main' : 'error.main' }}>{showDetail.balance.toLocaleString('fa-IR')} تومان</Typography>
              </Box>
            </Stack>
            <Divider sx={{ my: 2 }} />
            <Typography variant="subtitle2" sx={{ mb: 1 }}>آخرین تراکنش‌ها</Typography>
            {DEMO_TRANSACTIONS.filter(t => t.uid === showDetail.uid).slice(0, 3).map(t => (
              <Box key={t.id} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5 }}>
                ...
                <Typography variant="body2" sx={{ fontWeight: 600, color: t.type === 'UP' ? 'success.main' : 'error.main' }}>{t.type === 'UP' ? '+' : '-'}{t.amount.toLocaleString('fa-IR')}</Typography>
              </Box>
            ))}
          </DialogContent>
        ...
        <Dialog open onClose={() => setShowForm(false)} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            <Stack spacing={2} sx={{ mt: 1 }}>
              ...
              <Stack direction="row" sx={{flexWrap:"wrap"}} >
                <TextField label="نام" value={editUser.name} onChange={e => setEditUser({ ...editUser, name: e.target.value })} size="small" sx={{ flex: '1 1 150px' }} />
                <TextField label="خانواده" value={editUser.family} onChange={e => setEditUser({ ...editUser, family: e.target.value })} size="small" sx={{ flex: '1 1 150px' }} />
              </Stack>
              <Stack direction="row" sx={{flexWrap:"wrap"}} >
                <FormControl size="small" sx={{ flex: '1 1 150px' }}>
                  <InputLabel>گروه</InputLabel>
                  <Select value={editUser.group} label="گروه" onChange={e => setEditUser({ ...editUser, group: e.target.value })}>
                    {USER_GROUPS.map(g => <MenuItem key={g.id} value={g.id}>{g.name}</MenuItem>)}
                  </Select>
                </FormControl>
                <FormControl size="small" sx={{ flex: '1 1 150px' }}>
                  <InputLabel>دسترسی</InputLabel>
                  <Select value={editUser.role} label="دسترسی" onChange={e => setEditUser({ ...editUser, role: e.target.value })}>
                    {ACCESS_ROLES.map(r => <MenuItem key={r.id} value={r.id}>{r.name}</MenuItem>)}
                  </Select>
                </FormControl>
              </Stack>
              <FormControl fullWidth size="small">
                <InputLabel>دانشکده</InputLabel>
                <Select value={editUser.faculty} label="دانشکده" onChange={e => setEditUser({ ...editUser, faculty: e.target.value })}>
                  {FACULTIES.map(f => <MenuItem key={f.id} value={f.id}>{f.name}</MenuItem>)}
                </Select>
              </FormControl>
          ...
          <DialogActions>
            ...
            <Button variant="contained" onClick={() => { setShowForm(false); notify('ذخیره شد'); }}>ذخیره</Button>
          </DialogActions>
        </Dialog>
      )}
      ...
      {showResetPass && (
        <Dialog open onClose={() => setShowResetPass(null)} maxWidth="xs" fullWidth>
          ...
          <DialogContent>
            <TextField fullWidth size="small" label="رمز جدید" type="password" value={newPass} onChange={e => setNewPass(e.target.value)} sx={{ mt: 1 }} />
          </DialogContent>
          <DialogActions>
            <Button onClick={() => setShowResetPass(null)}>انصراف</Button>
            <Button variant="contained" color="warning" onClick={() => { setShowResetPass(null); setNewPass(''); notify('رمز ریست شد'); }}>ریست</Button>
          </DialogActions>
        </Dialog>
      )}
